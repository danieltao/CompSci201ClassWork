...
public class WordGramTester {
	...
	public void testShift() {
		...
		WordGram bs = a2.shiftAdd("hat");
		assertEquals("shift add",as.equals(b),true);
		assertEquals("shift add length",as.length() == a.length(),true);
		assertEquals("shift add",as.equals(b),true);
		assertEquals("shift add length",as.length() == a.length(),true);
		assertEquals("shift add",bs.equals(b2),true);
		assertEquals("shift add length",bs.length() == a2.length(),true);
		assertEquals("shift add",bs.equals(b2),true);
		assertEquals("shift add length",bs.length() == a2.length(),true);
	}
@Test
public void testToString(){
	...
	String bs = "zebra mongoose hat cat";
	String a2s = "apple zebra mongoose";
	String b2s = "";
	assertEquals("comp fail self",a.toString().equals(as) , true);
	assertEquals("comp fail copy",b.toString().equals(bs), true);
	assertEquals("fail sub", a2.toString().equals(a2s), true);
	//assertEquals("fail empty",b2.toString()==b2s, true);
	}
}
